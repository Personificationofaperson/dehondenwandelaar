"use client"

import { useEffect, useState } from "react"
import { whatsappLink } from "../lib/prijzen"
import { track, EVENTS } from "../lib/analytics"

const TEKST =
  "Hoi Kimberly! Ik heb een vraag over de hondenuitlaatservice. Het gaat om: [dienst]. Groetjes, [naam]"

export default function Whatsapp() {
  const [verborgen, setVerborgen] = useState(false)

  // De calculator heeft zijn eigen, grote WhatsApp-knop. Zolang die in beeld
  // staat verstopt de zwevende bel zich, zodat hij op mobiel niet over de
  // belangrijkste knop van de pagina valt.
  useEffect(() => {
    const doel = document.querySelector("#tarieven")
    if (!doel || typeof IntersectionObserver === "undefined") return
    const observer = new IntersectionObserver(
      ([entry]) => setVerborgen(entry.isIntersecting),
      { rootMargin: "-20% 0px -20% 0px" }
    )
    observer.observe(doel)
    return () => observer.disconnect()
  }, [])

  return (
    <a
      href={whatsappLink(TEKST)}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Stuur een bericht via WhatsApp"
      tabIndex={verborgen ? -1 : 0}
      aria-hidden={verborgen}
      onClick={() => track(EVENTS.CONTACT_KLIK, { plek: "zwevende-knop", kanaal: "whatsapp" })}
      className={`wa-fab fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] transition-all duration-300 hover:scale-105 ${
        verborgen ? "pointer-events-none translate-y-4 opacity-0" : "opacity-100"
      }`}
    >
      <svg width="30" height="30" viewBox="0 0 24 24" fill="#fff" aria-hidden="true">
        <path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.87 9.87 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0 0 12.04 2Zm0 18.15h-.01a8.2 8.2 0 0 1-4.19-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.2 8.2 0 0 1-1.26-4.38c0-4.54 3.7-8.23 8.25-8.23 2.2 0 4.27.86 5.83 2.41a8.19 8.19 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23Zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.25-.64.81-.79.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.13-.15.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.41-.42-.56-.43h-.48c-.17 0-.43.06-.66.31-.23.25-.86.85-.86 2.07 0 1.22.89 2.4 1.01 2.56.12.17 1.75 2.67 4.23 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.67-1.18.21-.58.21-1.07.15-1.18-.06-.11-.23-.17-.48-.29Z" />
      </svg>
    </a>
  )
}
